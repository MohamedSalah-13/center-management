...
public class CenterSettings {
    ...
    /*
     * التنبيهات: المفتاح الرئيسي وموعد الفحص اليومي.
     *
     * في إعدادات السنتر لا في تفضيلات الجهاز، تماماً كموعد النسخ الاحتياطي وللسبب
     * نفسه: هي سياسة واحدة للسنتر كله. ولو كانت على كل جهاز لَفحصت ثلاثة أجهزة في
     * ثلاثة مواعيد، ولَاختلف ما يراه الموظف على جهاز عمّا يراه على آخر.
     *
     * ضبط كل نوع على حدة - مفعَّل أم لا، لمن يذهب، بأي حدود - ليس هنا بل في جدول
     * alert_rules: أعمدةٌ باسم كل نوع كانت ستعني ترحيلاً جديداً مع كل نوع يُضاف.
     *
     * كلاهما يقبل null: قاعدة مُرقّاة من نسخة أقدم لن تحمل أياً منهما، والغائب يعني
     * السلوك القديم بالضبط - أي لا تنبيهات - فلا يبدأ البرنامج بعد ترقية في مراسلة
     * أحد أو ملء صندوق لم يطلبه صاحبه.
     */
    @Column(nullable = false)
    @Builder.Default
    private boolean alertsEnabled = false;

    /** ساعة الفحص اليومي؛ null يعني الافتراضي في {@code AlertSchedule.DEFAULT_TIME} */
    private LocalTime alertScanTime;

    /**
     * آخر فحص تنبيهات <b>مكتمل</b>. يُستعمل لتعويض موعد فات والجهاز مطفأ، ولتُظهر
     * الشاشة تاريخاً قديماً حين يفشل الفحص مرةً بعد مرة بدل أن يمرّ الفشل بصمت.
     */
    private LocalDateTime lastAlertScanAt;
