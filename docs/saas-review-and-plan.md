...
- [ ] **ب-4ب: التنبيهات والإشعارات** — قواعد التنبيهات، وصندوق الوارد، والفحص اليدوي،
  ورسائل أولياء الأمور. يسبقه `AlertRuleDraft`.

**والـ draft ليس زينة**: بقي `saveRule` وحده يأخذ كياناً، والقاعدة من المرحلة 3 صريحة - لا
كيان JPA مدخلاً لـ HTTP أبداً. فنصفُ كل دفعة عملٌ في `center-app` لا في `center-web`.
