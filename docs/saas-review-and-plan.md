...
- [x] **ب-1: تشغيل اليوم** — جدول اليوم، وفتحُ الحصص وإغلاقها، وقائمةُ المجموعات قراءةً.
  ...
  إلا سطح المكتب.
- [ ] **ب-2: الطلاب والاشتراكات** — يسبقه `StudentDraft` و`CourseGroupDraft`.
...
**والـ draft ليس زينة**: `saveStudent` و`saveGroup` و`saveTeacher` و`save(CenterSettings)`
و`saveRule` تأخذ كياناتٍ كما هي، والقاعدة من المرحلة 3 صريحة - لا كيان JPA مدخلاً لـ HTTP
أبداً. فنصفُ كل دفعة عملٌ في `center-app` لا في `center-web`.
