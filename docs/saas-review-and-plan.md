...
- [ ] **ب-4: الإدارة** — الإعدادات، المستخدمون، سجلّ المراقبة، قواعد التنبيهات، الإشعارات.
  يسبقه `CenterSettingsDraft` و`AlertRuleDraft` و`TeacherDraft`.

**والـ draft ليس زينة**: `saveTeacher` و`save(CenterSettings)` و`saveRule` ما زالت تأخذ
كياناتٍ كما هي، والقاعدة من المرحلة 3 صريحة - لا كيان JPA مدخلاً لـ HTTP أبداً. فنصفُ كل
دفعة عملٌ في `center-app` لا في `center-web`.
